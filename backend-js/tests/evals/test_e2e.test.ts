...
const SCORE_ANSWER_VS_CONTEXT_CORRECTNESS = "answer_vs_context_correctness_score";
...
function evaluateRetrievalRecall(
  ...
  const expectedSources = new Set(expectedOutput.sources || []);

  // Calculate recall - at least one expected source should be in retrieved docs
  const score = sources.some((source) => expectedSources.has(source)) ? 1.0 : 0.0;
...
describe("E2E Evaluation Tests", () => {
  it(
    "should pass regression test with minimum score thresholds",
    async () => {
      ...
      for (const example of examples) {
        console.log(`Evaluating: ${example.inputs.question}`);

        const actualOutput = await runGraph(example.inputs as { question: string });

        // Evaluate
        const qaScore = await evaluateQA(actualOutput, example.inputs, example.outputs);
      ...
      const avgContextCorrectness =
        results.reduce((sum, r) => sum + r.scores[SCORE_ANSWER_VS_CONTEXT_CORRECTNESS], 0) /
        results.length;

      console.log(`\nAverage Answer Correctness: ${avgAnswerCorrectness.toFixed(2)}`);
      console.log(`Average Context Correctness: ${avgContextCorrectness.toFixed(2)}`);

      // Assert minimum thresholds
      expect(avgAnswerCorrectness).toBeGreaterThanOrEqual(0.9);
      expect(avgContextCorrectness).toBeGreaterThanOrEqual(0.9);
    },
    { timeout: 300000 } // 5 minute timeout for full evaluation
  );
});

...
class DocsResearchGuardMiddleware(AgentMiddleware):
    ...
    async def awrap_model_call(
        ...
        if not self._should_retry(request, response):
            self._clear_attempts(run_key)
            return response

        while self._attempt_count(run_key) < _MAX_FORCED_ATTEMPTS:
            self._record_attempt(run_key)
    ...
    def _should_retry(self, request: ModelRequest, response: ModelResponse) -> bool:
        ...
        if self._instruction_in_history(request):
            return False
        ...
        if self._has_docs_research_tool(current_turn):
            return False
    ...
    def _run_key(self, request: ModelRequest) -> str:
        ...
        return f"turn:{self._turn_key(request.messages)}"
    ...
    def _has_docs_research_tool(self, messages: list[BaseMessage]) -> bool:
        return any(

...
**When quoting user-pasted code, NEVER echo API keys, tokens, or credentials verbatim.** Replace any secret-looking value with a placeholder like `YOUR_API_KEY_HERE`. Detect by common prefixes (`sk-`, `tvly-`, `AIza`, `ghp_`, `xoxb-`, `pk_live_`, `Bearer `, JWTs, LangSmith keys like `lsv2_` / `lcl_`, etc.), URI-userinfo credentials (`scheme://user:password@host`), or by contextual naming (`api_key=`, `token=`, `secret=`, `password=`, `LANGSMITH_API_KEY=`, `LANGCHAIN_API_KEY=`). When in doubt, redact. Leave any `<REDACTED_SECRET>` placeholder untouched when quoting user input back.

...
**CRITICAL: If 3 or more consecutive tool calls fail with the same error (e.g. 503, timeout), STOP retrying immediately. Tell the user the service is temporarily unavailable and suggest they try again later. Do NOT keep calling tools that are clearly down — this wastes time and resources.**

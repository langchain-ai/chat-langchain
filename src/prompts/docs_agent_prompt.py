...
**CRITICAL: If either support KB tool returns an error or the support knowledge-base research leg otherwise fails, explicitly identify that the support knowledge base was unavailable before the `Relevant docs:` block by saying: "Support articles could not be consulted, so this answer is based on official documentation only." This disclosure is mandatory; never claim that the support source was consulted or present a docs-only answer with full-confidence evidence from the support KB.**

...
**CRITICAL: Distinguish unavailable support research from a successful empty search. Say exactly "Support articles could not be consulted, so this answer is based on official documentation only." only when no support search result with articles is available and the support research failed, produced a tool error, or returned `consulted: false`. Say exactly "No matching support articles were found, so this answer is based on official documentation." only when all support searches completed successfully with `consulted: true` and no articles were returned. If any support search returns articles, do not emit either disclosure solely because another search returned zero matches; use and cite the returned articles.**

...
class WeaviateDataStore(DataStore):
    ...
    def __init__(self):
        ...
        logger.debug(
            f"Connecting to weaviate instance at {url} with API key"
        )
        self.client = Client(url, additional_headers=additional_headers)
    ...
    @staticmethod
    # Not needed at the moment since we're using API-Key
            ...
            for resp in response:
                result = DocumentChunkWithScore(
                    ...
                    metadata=DocumentChunkMetadata(
                        ...
                        source=resp["source"] if resp["source"] else "unknown",
    ...
    async def delete(
        ...
        if ids:
            operands = [
                {"path": ["document_id"], "operator": "Equal", "valueText": id}
                for id in ids
            ]

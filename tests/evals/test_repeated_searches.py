...
def test_prompt_requires_consistent_technical_recommendations():
    """Prompt must prevent unsupported reversals after failed technical advice."""
    prompt_lower = docs_agent_prompt.lower()

    assert "technical recommendations must be consistent across turns" in prompt_lower
    assert "when the user reports that prior advice produced an error" in prompt_lower

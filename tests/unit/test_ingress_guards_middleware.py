...
def test_before_agent_redacts_uri_password():
    middleware = IngressGuardsMiddleware()
    human = HumanMessage(
        content="postgres://u:S0meP4ssw0rd@127.0.0.1:5432/db?sslmode=disable",
        id="h1",
    )
    ...
    assert update["messages"][0].content == (
        "postgres://u:<REDACTED_SECRET>@127.0.0.1:5432/db?sslmode=disable"
    )


def test_before_agent_preserves_uri_placeholder_password():
    middleware = IngressGuardsMiddleware()
    content = "postgres://u:${PGPASS}@host/db"

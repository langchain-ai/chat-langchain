...
def test_invalid_request_error_without_status_is_not_retried():
    middleware = ModelRetryMiddleware(max_retries=2, initial_delay=0)
    calls = 0
    ...
    async def handler(request: ModelRequest):
        nonlocal calls
        calls += 1
        raise FakeInvalidRequestError("invalid request")

    with pytest.raises(FakeInvalidRequestError, match="invalid request"):
        asyncio.run(
            middleware.awrap_model_call(
                ModelRequest(model=object(), messages=[HumanMessage(content="Hi")]),
                handler,
...
def test_generic_error_is_retried():
    middleware = ModelRetryMiddleware(max_retries=2, initial_delay=0)
    calls = 0

...
@pytest.mark.parametrize("article_id", ["0", "N/A", "dummy", "nonexistent"])
def test_get_support_article_content_rejects_placeholder_ids(article_id):
    """Placeholder IDs are rejected before the article catalog is queried."""
...
def test_get_support_article_content_rejects_guessed_docs_path():
    """Documentation paths are not valid support article IDs."""
    with patch("src.tools.pylon_tools._fetch_all_articles") as fetch_articles:
        result = get_support_article_content.invoke(
            {"article_id": "/oss/python/langgraph/missing.mdx"}
        )

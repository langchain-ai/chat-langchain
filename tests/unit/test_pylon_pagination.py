...
class TestFetchAllArticlesPagination(unittest.TestCase):
    ...
    def setUp(self):
        ...
        pylon_module._pylon_auth_failure_until = 0.0
        pylon_module._pylon_auth_failure_status = None
    ...
    def test_unauthorized_response_is_sanitized_and_cached(
        self, mock_get, mock_kb_id, mock_api_key
    ):
        """Unauthorized article responses return a safe signal and are cached."""
        mock_response = MagicMock(status_code=401)
        mock_response.raise_for_status.side_effect = AssertionError("not expected")
        mock_get.return_value = mock_response

        first = self.module.search_support_articles.invoke({})
        second = self.module.search_support_articles.invoke({})

        self.assertEqual(first, self.module.PYLON_UNAVAILABLE_MESSAGE)
        self.assertEqual(second, self.module.PYLON_UNAVAILABLE_MESSAGE)
        self.assertEqual(mock_get.call_count, 1)
        self.assertNotIn("kb-123", first)
        self.assertNotIn("PYLON_API_KEY", first)

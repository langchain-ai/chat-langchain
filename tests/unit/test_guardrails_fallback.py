...
def test_guardrails_allowed_classification_resets_off_topic_flag(monkeypatch):
    """An allowed query should clear a previously set off-topic flag."""
    middleware = _middleware_with_models()
...
def test_guardrails_allowed_turn_clears_blocked_turn_state(monkeypatch):
    """An allowed turn should clear the flag after a blocked turn."""
    middleware = _middleware_with_models()
    decisions = iter(
        [
    ...
    async def _generate_rejection_message(content):  # noqa: ARG001
        return HumanMessage(content="This question is out of scope.")
    ...
    monkeypatch.setattr(middleware, "_track_decision_metadata", lambda decision: None)
    monkeypatch.setattr(middleware, "_add_to_dataset", lambda *args: asyncio.sleep(0))
    monkeypatch.setattr(guardrails_module.random, "random", lambda: 1)

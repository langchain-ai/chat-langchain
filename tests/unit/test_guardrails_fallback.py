...
def test_guardrails_allowed_query_clears_off_topic_flag(monkeypatch):
    """An allowed query should explicitly clear the persisted block flag."""
    middleware = _middleware_with_models()
...
def test_guardrails_allowed_turn_clears_previous_block(monkeypatch):
    """A later allowed turn should clear an earlier blocked turn's flag."""
    middleware = _middleware_with_models()
    decisions = iter(
        [
    ...
    async def _generate_rejection_message(content):  # noqa: ARG001
        return HumanMessage(content="Please ask a LangChain question.")

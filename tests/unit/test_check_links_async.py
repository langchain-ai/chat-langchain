...
@pytest.mark.asyncio
async def test_anchored_url_is_invalid_when_anchor_is_missing():
    """Anchored URLs require a matching element id or heading slug."""
    result = await _check_single_url(
        _FakeStreamingClient("<html><body><h2>Available section</h2></body></html>"),
        "https://example.com/docs#missing-section",
        timeout=1.0,
    )
...
@pytest.mark.asyncio
async def test_anchored_url_accepts_matching_heading_slug():
    """A heading whose slug matches the fragment makes the URL valid."""
    result = await _check_single_url(
        _FakeStreamingClient("<html><body><h2>Available section</h2></body></html>"),
        "https://example.com/docs#available-section",
        timeout=1.0,
    )

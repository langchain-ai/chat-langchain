...
def test_inline_urls_are_validated_and_invalid_urls_are_removed(monkeypatch):
    from src.middleware import citation_guard_middleware as citation_module
    from src.middleware.citation_guard_middleware import CitationGuardMiddleware
    from src.tools.link_check_tools import LinkCheckResult

    grounded = "https://docs.langchain.com/oss/python/langgraph/graph-api"
    invalid = "https://bad.example/reference"
    checks: list[list[str]] = []
    middleware = CitationGuardMiddleware()
    request = ModelRequest(
        model=object(),
    ...
    async def handler(request: ModelRequest) -> ModelResponse:
        return ModelResponse(
            result=[
                AIMessage(
                    content=f"Use the guide at ({grounded}); avoid ({invalid})."
                )
            ]
        )
    ...
    assert invalid not in result.result[0].content


def test_missing_citations_retries_when_docs_results_contain_urls():
    from src.middleware.citation_guard_middleware import CitationGuardMiddleware
    ...
    async def handler(request: ModelRequest) -> ModelResponse:
        calls.append(request)
        return ModelResponse(result=[AIMessage(content="Use a graph.")])
...
def test_missing_citations_without_docs_results_passes_through():
    from src.middleware.citation_guard_middleware import CitationGuardMiddleware

    middleware = CitationGuardMiddleware()
    calls: list[ModelRequest] = []
    request = ModelRequest(
        model=object(), messages=[HumanMessage(content="Hello there.")]
    )
    ...
    assert len(calls) == 1
    assert result.result[0].content == "Hello!"
